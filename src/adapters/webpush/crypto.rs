...
impl CryptoContext {
    ...
    fn aesgcm128_auth_data(&self, auth: &Option<String>, peer_key: &[u8], encrypt: bool) -> Option<AuthData> {
        ...
        let mut key_context : Vec<u8> = Vec::with_capacity(peer_key.len() + local_key.len() + 5);
        if encrypt {
    ...
    fn aesgcm128_common(&self, salt: &[u8], shared_key: &[u8], auth: Option<AuthData>) -> ([u8; 32], [u8; 32], [u8; 12]) {
        ...
        match auth {
            Some(ad) => {
                ...
                encrypt_info.extend_from_slice(b"Content-Encoding: aesgcm\x00P-256\x00");
                encrypt_info.extend_from_slice(&ad.key_context);
                nonce_info.extend_from_slice(b"Content-Encoding: nonce\x00P-256\x00");
